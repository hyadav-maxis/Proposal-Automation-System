...
export default function Proposals() {
  ...
  const [minDbSize, setMinDbSize] = useState('');
  const [maxDbSize, setMaxDbSize] = useState('');
  ...
  async function loadProposals(loc = 'all', q = '', minSz = '', maxSz = '') {
    setLoading(true);
    setError(null);
    try {
      ...
      if (minSz && minSz !== '') params.append('min_db_size', minSz);
      if (maxSz && maxSz !== '') params.append('max_db_size', maxSz);
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button 
            ...
            onClick={() => {
              ...
              if (minDbSize) url += `&min_db_size=${minDbSize}`;
              if (maxDbSize) url += `&max_db_size=${maxDbSize}`;
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  <span style={{ position: 'absolute', left: 10, fontSize: '0.75rem', color: 'var(--text-muted)', pointerEvents: 'none' }}>≥</span>
                  <input 
                    type="number" 
                    placeholder="Min GB"
                    value={minDbSize} 
                ...
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600, marginLeft: 2 }}>GB</span>
              </div>
            </div>
