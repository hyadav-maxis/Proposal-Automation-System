...
def _resolve_logo(logo_path: Optional[str] = None) -> Optional[str]:
    ...
    # Walk up two levels to reach the project root (app/services → app → root)
...
class ExportService:
    ...
    def generate_pdf(
        ...
        res_loc = (proposal_data.get("resource_location") or "").replace("_", " ").title()
        ...
        for key in sorted(pricing.keys()):
            ...
            if key == "usa_resources_surcharge":
                continue
        ...
        # Surcharge
        surcharge = pricing.get("usa_resources_surcharge")
