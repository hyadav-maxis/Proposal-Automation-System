...
class PricingService:
    ...
    def calculate_database_size_price(self, size_gb: float, us_based_resources: bool = False) -> float:
        ...
        price = base_price + variable_price
        if us_based_resources:
            us_config = self._get_config("us_resource_pricing")
            m = Decimal(str(us_config.get("surcharge_multiplier", 1.35)))
            price = float(round(Decimal(str(price)) * m, 2))
        return price

    def calculate_runs_price(self, num_runs: int, us_based_resources: bool = False) -> float:
        ...
        price = base_price + total_runs_price
        if us_based_resources:
            us_config = self._get_config("us_resource_pricing")
            m = Decimal(str(us_config.get("surcharge_multiplier", 1.35)))
            price = float(round(Decimal(str(price)) * m, 2))
    ...
    def calculate_deployment_price(self, deployment_type: str, us_based_resources: bool = False) -> float:
        config = self._get_config("deployment_pricing")
        price = float(config.get(deployment_type, 0))
        if us_based_resources:
            us_config = self._get_config("us_resource_pricing")
            m = Decimal(str(us_config.get("surcharge_multiplier", 1.35)))
            price = float(round(Decimal(str(price)) * m, 2))
    ...
    def calculate_total_price(
        ...
        us_based_resources: bool = False,
    ) -> Dict[str, Any]:
        """
        Compute all price components and return a full breakdown dict.
        """
        db_price = self.calculate_database_size_price(database_size_gb, us_based_resources)
        runs_price = self.calculate_runs_price(num_runs, us_based_resources)
        deployment_price = self.calculate_deployment_price(deployment_type, us_based_resources)
        where_price = self.calculate_where_clauses_price(has_where_clauses)
        if us_based_resources:
            us_config = self._get_config("us_resource_pricing")
            m = Decimal(str(us_config.get("surcharge_multiplier", 1.35)))
            where_price = float(round(Decimal(str(where_price)) * m, 2))

        maximo_price = self.calculate_maximo_upgrade_price(has_maximo_upgrade, maximo_has_addon)
        if us_based_resources:
            us_config = self._get_config("us_resource_pricing")
            m = Decimal(str(us_config.get("surcharge_multiplier", 1.35)))
            maximo_price = float(round(Decimal(str(maximo_price)) * m, 2))
        ...
        if us_based_resources:
            us_config = self._get_config("us_resource_pricing")
            m = Decimal(str(us_config.get("surcharge_multiplier", 1.35)))
            for k in ["db2_installation_price", "birt_installation_price", "maximo_installation_price", "total"]:
                addon_prices[k] = float(round(Decimal(str(addon_prices[k])) * m, 2))
        ...
        if us_based_resources:
            us_config = self._get_config("us_resource_pricing")
            m = Decimal(str(us_config.get("surcharge_multiplier", 1.35)))
            birt_price = float(round(Decimal(str(birt_price)) * m, 2))
            if birt_breakdown_data:
                for score in birt_breakdown_data:
                    bd = birt_breakdown_data[score]
                    bd["price_per_report"] = float(round(Decimal(str(bd["price_per_report"])) * m, 2))
                    bd["total_price"] = float(round(Decimal(str(bd["total_price"])) * m, 2))
        ...
        # Surcharge line item is now omitted because it's baked into individual components
        result["us_resources_surcharge"] = 0.0
        if us_based_resources:
            us_config = self._get_config("us_resource_pricing")
            result["us_resources_multiplier"] = float(us_config.get("surcharge_multiplier", 1.35))
